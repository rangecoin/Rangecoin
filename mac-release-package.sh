...
PACKAGE=RNG-client-${VERSION}
echo PACKAGE="${PACKAGE}"
CHANGELOG=RNG-client-${VERSION}.changelog.txt
OBFUSCATE=$2
MACVERSION=$3
if [ -x ${MACVERSION} ];
then
...
FILES="${FILES} Rangecoin.exe Rangecoinservice.exe"
...
echo copy resources
cp installer/lib/JavaExe.exe Rangecoin.exe
cp installer/lib/JavaExe.exe Rangecoinservice.exe
cp -a ${FILES} nxt
